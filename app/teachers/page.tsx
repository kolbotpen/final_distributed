...
function CreateTeacherModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", department: "", status: "active" });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-bold text-gray-900">New Teacher</h2>
...
export default function TeachersPage() {
  ...
  return (
    <div className="space-y-6">
      {showCreate && (
        <CreateTeacherModal onClose={() => setShowCreate(false)} onCreated={load} />
      )}
      ...
      <div className="flex gap-3">
        ...
        <button disabled={teachers.length < limit} onClick={() => setOffset(offset + limit)} className="px-4 py-2 rounded-lg bg-indigo-100 text-indigo-700 disabled:opacity-40">Next →</button>
      </div>
    </div>
  );
}
